...
- Mobile hardening smoke verifies the xterm first row is unobstructed (the redundant absolute CONNECTED badge is removed), each pane collapses desktop color/AI/tools actions into one right-aligned mobile overflow menu, and xterm refits after active/fullscreen/soft-keyboard/VisualViewport/orientation changes. Simulated safe areas cover Dynamic-Island/notch + home-indicator geometry in portrait, CSS/native fullscreen fallback, fixed Broadcast/settings surfaces and landscape side insets across 347×729, 390×844, 430×932 and 844×390 with no page overflow.
...
| `pane-header.tsx` + `pane-menu-cluster.tsx` + non-duplicated action subset | `src/lib/features/terminals/PaneChrome.svelte` + `PaneMobileMenu.svelte` + `PaneToolsMenu.svelte` — desktop quick actions remain direct, while mobile collapses Color/AI/Skills+folders/workspace/font/focus/fullscreen/duplicate/close into one right-aligned overflow sheet; profile/title, RTT/activity and session-color SSOT stay visible without squeezing the terminal. Safe-area geometry and nested controlled menus are browser-verified. |
...
- [x] `pane-menu-cluster.tsx` mobile behavior — `PaneMobileMenu.svelte` now provides one right-aligned mobile overflow trigger for Color/AI/Skills+folders/workspace/font/focus/fullscreen/duplicate/close, while desktop retains direct quick controls. Nested menus are controlled from the same mobile cluster and safe-area/browser verified. Any future pane action must still be checked for genuinely unique behavior before adding another source.
