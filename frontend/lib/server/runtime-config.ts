...
export function buildRuntimeConfigResponse(config = readRuntimeConfig()): RuntimeConfigResponse {
  const environmentMap = new Map(config.environments.map((environment) => [environment.id, environment]));

  const environments: RuntimeEnvironmentSummary[] = config.environments.map((environment) => {
  ...
  const agentProfiles: RuntimeResolvedAgentProfile[] = config.agentProfiles.map((profile) => ({
    ...profile,
