// `node:` prefix forces the Node builtin. A bare 'crypto' import gets pulled
// into the Edge bundle by Next's module tracer and fails to build on a fresh
// install; the prefix marks it node-only.
...
export function verifySession(cookie: string, secret: string): SessionPayload | null {
  try {
    ...
    // cookie (the HMAC key would be empty/guessable). Never accept a session
    // under a weak key — reject every cookie instead of validating against it.
