  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      socket.on('message', (message) => {
        if (closed) return;
        // Backpressure: if the browser stalls, the ReadableStream queue grows
        // unbounded in this process. desiredSize goes increasingly negative as
        // undrained chunks pile up; past the cap, drop the stream so the client
        // reconnects and re-bootstraps instead of letting memory balloon.
