...
function mergeAppMetadata(
  base: AppMetadata | undefined,
  incoming: AppMetadata | undefined
  ...
  return {
    ...
    ...(incoming ?? {}),
    command_targets: {
      ...(base?.command_targets ?? {}),
      ...(incoming?.command_targets ?? {}),
    },
  };
}
  ...
  for (const partial of dokployApps) {
    ...
    if (existing) {
      appMap.set(partial.name, {
        ...
        source: 'dokploy',
        metadata: mergeAppMetadata(existing.metadata, partial.metadata as AppMetadata | undefined),
      } as AppStatus);
      continue;
    }
    ...
    if (partial.runtime_status && partial.health_status && partial.ports) {
      appMap.set(partial.name, {
        ...(partial as AppStatus),
        metadata: mergeAppMetadata(undefined, partial.metadata as AppMetadata | undefined),
      });
    }
  }
...
export async function runAppCollectorTask(): Promise<void> {
  try {
    ...
    updateKnownTargets(
      'container',
      dockerApps
        .map((app) => app.metadata?.command_targets?.container)
        .filter((target): target is string => typeof target === 'string' && target.length > 0)
    );
    updateKnownTargets(
      'dokploy-app',
      dokployApps
        .map((app) => (app.metadata as AppMetadata | undefined)?.command_targets?.dokploy_project)
        .filter((target): target is string => typeof target === 'string' && target.length > 0)
    );
