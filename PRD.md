...
Canonical local path: `<your-repo-path>`  
...
(Replace with your actual container/app names)
...
Type=simple
User=<your-user>
WorkingDirectory=<your-repo-path>/frontend
EnvironmentFile=<your-repo-path>/.env.local
...
Catatan: Frontend dijalankan dengan `npm run start` (bukan standalone server.js). Lihat `scripts/install-systemd.sh` untuk unit file yang up-to-date.
...
Type=simple
User=<your-user>
WorkingDirectory=<your-repo-path>/agent
EnvironmentFile=<your-repo-path>/.env.local
ExecStart=/usr/bin/node <your-repo-path>/agent/dist/index.js
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
...
App user harus masuk ke group `docker`:
